...
#      tax_year, row_seq, section, row_label, col_seq, col_label, value,
#      flag, unit
...
#    pair keys the same cell across years. aligned/modern/_coverage.csv
...
extract_sheet = function(path) {
  m   = read_sheet_matrix(path, script_dir)
  loc = find_numrow(m)
  if (is.null(loc)) {
    loc = find_data_block(m, FIRST_ITEM_REGEX)
    hdr_end = loc$row          # 'row' is already the line above the data
  } else {
    hdr_end = loc$row - 1      # exclude the column-number row itself
  }
  headers = stack_headers(m, hdr_end, loc$cols)

  label_cols = seq_len(min(loc$cols) - 1)
  min_cells  = max(2, ceiling(0.1 * length(loc$cols)))
  ...
  for (i in (loc$row + 1):nrow(m)) {
    ...
    label_raw = rev(lab_cells[lab_cells != ''])[1]
    if (is.na(label_raw)) label_raw = ''
